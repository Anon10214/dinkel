...
func TestIntegrationModels(t *testing.T) {
	...
	for _, model := range []string{
		...
		t.Run("Test "+model, func(t *testing.T) {
			...
			conf, err := config.GetConfigForTarget(model, "../targets-config.yml")
			if assert.NoError(t, err, "Getting config for target failed") {
				...
				t.Run("GetSchema works", func(t *testing.T) {
					t.Run("Node labels fetched correctly", func(t *testing.T) {
						...
						assert.Len(t, s.Labels[schema.RELATIONSHIP], 0, "More than 0 relationship labels fetched")
						if !assert.Len(t, s.Labels[schema.NODE], 1, "More or less than 1 node labels fetched") {
							t.FailNow()
						}
