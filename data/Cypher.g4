...
oC_Foreach
     : FOREACH SP? '(' SP? oC_IdInColl SP? '|' SP? (oC_UpdatingClause SP?)+ ')' ;
...
oC_ExistentialSubquery
                   :  EXISTS SP? '{' SP? ( oC_RegularQuery | ( oC_Pattern ( SP? oC_Where )? ) | ( ( ( oC_ReadingClause | oC_With ) SP? )+ ) ) SP? '}' ;

EXISTS : ( 'E' | 'e' ) ( 'X' | 'x' ) ( 'I' | 'i' ) ( 'S' | 's' ) ( 'T' | 't' ) ( 'S' | 's' ) ;

oC_CountSubquery
                   :  COUNT SP? '{' SP? ( oC_RegularQuery | ( oC_Pattern ( SP? oC_Where )? ) | ( ( ( oC_ReadingClause | oC_With ) SP? )+ ) ) SP? '}' ;
